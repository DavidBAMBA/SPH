...
void GhostParticles(std::vector<Particle>& mesh, double x1, double x2) {
    
    for (int ii = 0; ii < mesh.size(); ++ii) {
        if (mesh[ii].r < x1) {

            Particle ghost = mesh[ii];
            ghost.r = 2 * x1 - mesh[ii].r; // Reflejar la posición
            ghost.v = -mesh[ii].v; // Invertir la velocidad para la condición de no-penetración
            mesh.push_back(ghost);

        } else if (mesh[ii].r > x2) {
            Particle ghost = mesh[ii];
            ghost.r = 2 * x2 - mesh[ii].r; // Reflejar la posición
            ghost.v = -mesh[ii].v; // Invertir la velocidad
            mesh.push_back(ghost);
        }
    }
}
