...
def plot_system(W_i):
    ...
    # Density plot
    axs[0, 0].scatter(W_plot[:,0], W_plot[:,1])
    ...
    # Velocity plot
    axs[0, 1].scatter(W_plot[:,0], W_plot[:,2])
    ...
    # Energy plot
    axs[1, 0].scatter(W_plot[:,0], W_plot[:,3])
    ...
    # Pressure plot
    axs[1, 1].scatter(W_plot[:,0], W_plot[:,4])
    axs[1, 1].set_xlim([-0.4, 0.4])
    axs[1, 1].set_title('Pressure plot')
    axs[1, 1].set_xlabel('$Position $')
    axs[1, 1].set_ylabel('$Pressure $')

    plt.savefig('mi_grafica2.png', dpi=300)  # Guarda la gráfica como una imagen PNG con una resolución de 300 dpi
...
# Evenly spaced positions along the 1D domain
x = np.linspace(-1, 1, N)  # Domain from -1 to 1

# Initial density for the particles (homogeneous)
rho = np.full(N, 1)  # Uniform density

# Initial energy, with a high-energy region in the center
e = np.ones(N)
center_region = (x > 0.0) & (x < 0.1)  # Define the region for the blast (e.g., -0.1 to 0.1)
e[center_region] = 100  # Assign high energy to the center region to simulate the blast

# Initial velocities are zero everywhere
v = np.zeros(N)

# Initial pressures - higher in the center where the blast is
p = np.zeros(N)
p[center_region] = (1.7 - 1) * rho[center_region] * e[center_region]  # Using ideal gas law

# Create the initial state vector as a matrix
W = np.zeros((N, 5))  # Matrix to store initial conditions
W[:,0] = x
W[:,1] = rho
W[:,2] = v
W[:,3] = e
...
# Flatten the matrix into a vector if needed
...
def integrate(t, W):
    ...
    for i in range(N - 1):    
        for j in range(i+1, N):
            ...
            if np.linalg.norm(dx) <= kappa*h_pair:
                # Store indexes 
                #print('pair: ', i, 'and ', j, ' with q: ', smoothingW(dx, h_pair), ' and dq: ', smoothingdW(dx, h_pair), '\n')
                pair_i.append(i)
                pair_j.append(j)
    ...
    for k in range(npairs):         
        ...
        #print("density update for pairs ", pair_i[k], ' and ', pair_j[k], ': ', W[pair_i[k],1], ' and ', W[pair_i[k],1], '\n')
    ...
    for k in range(npairs):        
        ...
        #print('Acceletarion for pair ', pair_i[k], ' and ', pair_j[k], ': ',dW[pair_i[k], 2], ' and ', dW[pair_j[k], 2], '\n')
        #print('Energy for pair ', pair_i[k], ' and ', pair_j[k], ': ',dW[pair_i[k], 3], ' and ', dW[pair_j[k], 3], '\n')
...
'''
def Real_Time_PLot(W):

    fig, ((ax1, ax2), (ax3, ax4)) = plt.subplots(2, 2, figsize=[10,10])

    line1, = ax1.plot([], [], '.', lw=1)
    ax1.set_xlim([-0.4, 0.4])
    ax1.set_title('Density 1')
    ...
    def update(i):
        W_plot = W_i[i]
        line1.set_data(W_plot[:,0], W_plot[:,1])
        line2.set_data(W_plot[:,0], W_plot[:,2]) # Cambiado para demostración, puedes adaptar según lo que necesites
        line3.set_data(W_plot[:,0], W_plot[:,3]) # Cambiado para demostración
        line4.set_data(W_plot[:,0], W_plot[:,4]) # Cambiado para demostración
        return line1, line2, line3, line4,
