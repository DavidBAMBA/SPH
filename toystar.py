...
def PairwiseSeparations(ri, rjj):
	"""
	Get pairwise separations between 2 sets of coordinates
	rii: M x 3 matrix of positions
	rjj : N x 3 matrix of positions
	...
	# other set of points positions rjj = (x,y,z)
	rjjx = rjj[:,0].reshape((1,N))
	rjjy = rjj[:,1].reshape((1,N))
	rjjz = rjj[:,2].reshape((1,N))
	
	# matrices that store all pairwise particle separations: r_ii- r_jj
	dx = rix - rjjx
	dy = riy - rjjy
	dz = riz - rjjz
...
def Acceleration( pos, vel, m, h, k, n, lmbda, nu ):
	...
	# Calculate densities at the position of the p
